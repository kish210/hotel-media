...
## Venues & Signage — صفحه‌های محیط عمومی هتل

لابی، رستوران، جلوی سالن کنفرانس، استخر (migration `026`).

### مسئله‌ای که حل می‌کند

صفحه‌نمایش فقط یک «نام» داشت؛ سیستم نمی‌دانست کجا نصب شده. نتیجه: برای
هر در سالن باید یک پلی‌لیست جدا ساخته می‌شد. و هرچه برای تلویزیون اتاق
ساخته شده بود — منوی تصویری، اخبار، دفترچه تلفن، نوار زنده — برای صفحه‌های
محیط عمومی در دسترس نبود، چون پلی‌لیست فقط **فایل آپلودشده** می‌پذیرفت.

حالا صفحه به **محل** وصل می‌شود و یک پلی‌لیست واحد روی همه‌ی درها کار
می‌کند: هر صفحه برنامه‌ی سالن خودش را نشان می‌دهد.

### محل‌ها

| مسیر | کار |
|------|-----|
| `GET|POST /venues` | فهرست و ساخت |
| `PUT|DELETE /venues/{id}` | ویرایش و حذف |
| `POST /venues/{id}/assign-screen` | اتصال صفحه‌نمایش به محل |

**انواع:** `lobby` · `reception` · `restaurant` · `cafe` · `hall` · `pool` ·
`spa` · `gym` · `elevator` · `corridor` · `shop` · `other`

هر محل می‌تواند ساعت کاری و یک **منوی تصویری پیش‌فرض** داشته باشد — صفحه‌ی
رستوران خودکار منوی همان رستوران را نشان می‌دهد.

حذف محل، صفحه‌ها و رویدادها را حذف نمی‌کند؛ فقط ارجاعشان خالی می‌شود.

### رویدادهای سالن

| مسیر | کار |
|------|-----|
| `GET /events` | فیلتر با `venue_id`، `from`، `to`، `upcoming` |
| `POST /events` · `PUT|DELETE /events/{id}` | مدیریت |

**تداخل رزرو بررسی می‌شود:** دو رویداد همزمان در یک سالن با `409` رد
می‌شوند. رویداد بدون زمان پایان، دو ساعته فرض می‌شود — همان فرضی که تابلو
دارد. رویداد لغوشده تداخل نمی‌سازد.

**وضعیت:** `scheduled` · `ongoing` · `ended` · `cancelled`

### آیتم‌های پویا در پلی‌لیست

`playlist_items.item_type` مشخص می‌کند آیتم فایل است یا محتوای پویا:

| نوع | چه نشان می‌دهد |
|-----|----------------|
| `media` | فایل آپلودشده (پیش‌فرض) |
| `event_board` | برنامه‌ی سالنِ همین صفحه — بدون محل، همه‌ی سالن‌ها (تابلوی لابی) |
| `menu_board` | منوی تصویری محل، یا منوی مشخص‌شده در `ref_id` |
| `news` | اخبار از `content_items` |
| `directory` | شماره‌های داخلی |
| `venue_info` | ساعت کاری، ظرفیت، رویداد جاری محل |
| `info_bar` | ساعت، آب‌وهوا، نرخ ارز، اوقات شرعی |
| `live_tv` | کانال زنده (کانال بزرگسال هرگز روی صفحه عمومی نمی‌آید) |

**آیتمی که الان چیزی برای نمایش ندارد، از پلی‌لیست حذف می‌شود** — سالنی
بدون رویداد امروز نباید پلیر را روی یک صفحه‌ی خالی نگه دارد.

تابلوی رویداد فقط **امروز و فردا** را نشان می‌دهد؛ تابلوی جلوی در نباید
برنامه‌ی هفته‌ی بعد را نمایش دهد.

یک ویجت خراب کل پخش را قطع نمی‌کند — خطایش لاگ و آیتم رد می‌شود.
