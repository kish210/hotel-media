...
window.doActivate = async function() {
  ...
  if (!code) { err.textContent = 'کد فعال‌سازی را وارد کنید'; return; }
  ...
  try {
    ...
    if (d.success) {
      ...
      err.textContent = '✅ ' + (d.message || 'فعال‌سازی موفق');
      setTimeout(() => window.location.reload(), 1200);
    } else if (r.status === 422) {
      err.style.color = '#ef4444';
      err.textContent = '❌ ' + (d.message || 'کد نامعتبر یا منقضی شده');
    } else {
      err.style.color = '#ef4444';
      err.textContent = '❌ خطا: ' + (d.message || r.status);
    }
  } catch(e) {
    err.style.color = '#f59e0b';
    err.textContent = '⚠ خطا در اتصال — آدرس سرور: ' + SERVER_URL;
  }
};
