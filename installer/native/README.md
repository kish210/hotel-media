...
run by double-clicking. It installs and starts Hotel Media completely — with **no
...
1. Double-click **`HotelMedia-vX.Y.Z-setup.exe`**.
...
| | |
|---|---|
| **Login** | `admin@hotelmedia.com` |
| **Password** | `Admin@123456` *(change it after first login)* |

Start menu → **Hotel Media** has shortcuts to open the dashboard, start/stop the
...
4. Compile `HotelMedia-Native.iss` into:

```
installer\native\Output\HotelMedia-vX.Y.Z-setup.exe
```
...
The installer copies everything to `C:\Hotel Media` and then runs
[`runtime/provision.ps1`](runtime/provision.ps1), which:

1. Generates `.env` with random secrets (DB password, `APP_KEY`, JWT).
2. Initializes a private MariaDB data directory in `C:\Hotel Media\data\mysql`.
3. Registers MariaDB as the **`Hotel Media-MySQL`** service and starts it.
...
5. Registers two more services via NSSM:
   - **`HotelMedia-Web`** — `php -S 0.0.0.0:80` with `public/server-router.php`
   - **`HotelMedia-WS`** — the pure-PHP WebSocket server
...
| File | Purpose |
|------|---------|
| `HotelMedia-Native.iss` | Inno Setup script (defines the installer) |
...
| `runtime/Manage-HotelMedia.ps1` | start / stop / restart / status helper |
